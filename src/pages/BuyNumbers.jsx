...
const BuyNumbers = ({ darkMode }) => {
  ...
  const handleBuy = async (service, callback) => {
    if (!selectedCountry) return alert("Please select a country first!");
    if (balance < service.priceNGN) return alert("Insufficient balance");

    await debitWallet(service.priceNGN);
    ...
    try {
      const res = await axios.post(
        ...
        {
          country: selectedCountry.short_name || selectedCountry.ID,
          service: service.ID,        // numeric ID
          pool: "default",
          max_price: service.priceUSD, // USD directly
          quantity: 1,
        },
        { headers: { Authorization: `Bearer ${token}` } }
      );
            ...
            {loadingServices ? (
              ...
              <div className="services-grid">
                {filteredServices.map((service) => (
                  <ServiceCard
                    key={service.ID}
                    service={service} // includes priceNGN and priceUSD
                    onBuy={handleBuy}
                    darkMode={darkMode}
                    disabled={balance < service.priceNGN}
                  />
                ))}
              </div>
            )}
