...
const USD_TO_NGN = 1000; // must match backend
  ...
  useEffect(() => {
    const fetchServices = async () => {
      ...
      try {
        ...
        // Optional: filter by selected country if needed
        const filtered = allServices.filter(
          (s) =>
            !s.countryID ||
            s.countryID === selectedCountry.ID ||
            s.countryShort === selectedCountry.short_name
        );
    ...
    try {
      const res = await axios.post(
        `${API_URL}/api/smspool/buy`,
        {
          ...
          max_price: service.price / USD_TO_NGN, // ✅ convert NGN → USD
          quantity: 1,
        },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      // ✅ Debit wallet only AFTER successful purchase
      await debitWallet(service.price);
              ...
              <div className="services-grid">
                {filteredServices.map((service) => (
                  <ServiceCard
                    ...
                    onBuy={handleBuy}
                    darkMode={darkMode}
                    disabled={!service.price || balance < service.price}
                  />
                ))}
              </div>
            )}
