...
const OpayFund = () => {
  ...
  const handleQuickAmount = (value) => setAmount(value);
  ...
  const handleFund = async () => {
    ...
    if (amount < MIN_AMOUNT) {
      setError(`Minimum funding amount is ${MIN_AMOUNT} NGN.`);
      return;
    }

    if (amount > MAX_AMOUNT) {
      setError(`Maximum funding amount is ${MAX_AMOUNT} NGN.`);
      return;
    }
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/opay/init`, {
        method: "POST",
        headers: {
  ...
  return (
    <div className="opay-fund-wrapper">
      <h2>Fund Wallet</h2>
